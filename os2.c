...
void lis(char **argv,int flag)								//for all the other commands without redirection and pipes
{
	pid_t pid;
	int status;
	pid=fork();
...
void kill_it()												//to kill all the background processes
{
	int i=0,j;
	for(i=0;i<count;i++)
	{
...
void child_handler(int signum)										//child handler
{
	int pid;
...
void get_status (pid_t pid)									//to get status of a job
{
  char procpath[100];
  char buf[100];
  int pfd;
